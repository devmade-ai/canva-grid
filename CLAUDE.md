...
- **pdf-lib image handling:** pdf-lib embeds PNG directly (FlateDecode — no re-encoding). Digital PDF export uses pixelRatio:2 with 1:1 pixel-to-point mapping (page dims = 2× platform pixels) for sharp phone display. Print formats use pixelRatio:1 with 72/150 conversion. History: Original pixelRatio:2 + 72/96 caused 2.667:1 non-integer ratio destroying gradients. First fix overcorrected to pixelRatio:1 (72 DPI, blurry on phones). Current fix: 2× capture with 1:1 mapping = no resampling + 4× more pixels. Diagnostic image download enabled in dev mode.
