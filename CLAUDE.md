...
- **pdf-lib image handling:** pdf-lib embeds PNG directly (FlateDecode — no re-encoding). Digital PDF export captures at user-selected pixelRatio (1x/2x/3x) but keeps page at platform pixel dimensions. Higher pixelRatio = more pixels per point = sharper on mobile (2:1 at standard, 3:1 at high). Print formats use pixelRatio:1 with 72/150 conversion. History: Original pixelRatio:2 + 72/96 caused 2.667:1 non-integer ratio destroying gradients. Second attempt used 1:1 pixel-to-point mapping but scaled page WITH pixelRatio — all qualities looked identical on mobile (oversized page just got scaled back down). Current fix: fixed page size + variable pixelRatio = real quality differences. Diagnostic image download enabled in dev mode.
